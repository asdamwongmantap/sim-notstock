<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <!-- Meta, title, CSS, favicons, etc. -->
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
	<link rel="icon" href="{{ asset('/assets/images/logoawr.jpg')}}" type="image/ico" />

<title>SIM-NOTSTOCK </title>

	<script src="{{ asset('/assets/js/jquery.js')}}"></script>
    <!-- Bootstrap -->
    <link href="{{ asset('/assets/vendors/bootstrap/dist/css/bootstrap.min.css')}}" rel="stylesheet">
    <!-- Font Awesome -->
    <link href="{{ asset('/assets/vendors/font-awesome/css/font-awesome.min.css')}}" rel="stylesheet">
	<!-- Select2 -->
    <link href="{{ asset('/assets/vendors/select2/dist/css/select2.min.css')}}" rel="stylesheet">
    <!-- Switchery -->
    <link href="{{ asset('/assets/vendors/switchery/dist/switchery.min.css')}}" rel="stylesheet">
	<!-- Custom Theme Style -->
    <link href="{{ asset('/assets/build/css/custom.min.css')}}" rel="stylesheet">
	<!-- sweet alert -->
	<link rel="stylesheet" href="{{ asset('/assets/alert/css/sweetalert.css')}}">
	<!-- bootstrap-datetimepicker -->
<link href="{{ asset('/assets/vendors/bootstrap-datetimepicker/build/css/bootstrap-datetimepicker.css')}}" rel="stylesheet">
<link rel="stylesheet" href="{{ asset('/assets/js/jquery-ui.css')}}">

<script type="text/javascript" src="{{ asset('/assets/chartjs/Chart.js')}}"></script>
